  ...
  data () {
    return {
      ...
      exampleData: [
        ...
        '-----',
        'table：data、sortBy、sortOrder、tableClass、theadClass、tbodyClass、show-header、showColumns（Array）, slot="empty-text"，cache(Boolean)：是否缓存表格的列配置，默认开启，cacheKey：自定义key,showOption(Boolean)配置表格，默认为显示，inside(Boolean)：筛选框在表格外部还是内部',
        'table-column：prop、label、hidden、sortable、dataType、cellClass、headerClass、align、formatter、sortBy、width、min-width、selectable',
