<template>
  <ul v-if="shouldShowPagination" class="pagination justify-content-center">
    <li>
      <a :class="{ disabled: gotoPreviousDisabled }" @click="gotoPreviousPage()">《</a>
    </li>
...
export default {
  ...
  methods: {
    ...
    renderPageWithEllipsisBlocks ({ pageBlock }) {
      ...
      const middleIndexElement = Math.ceil(maxPageBlocks / 2)
